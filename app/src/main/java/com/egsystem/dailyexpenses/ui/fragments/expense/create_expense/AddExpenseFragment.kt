package com.egsystem.dailyexpenses.ui.fragments.expense.create_expense
...
class AddExpenseFragment : Fragment(R.layout.fragment_add_expense),
    ...
    private var expenseCategory = "Not categorised"
    private var drawableSelected = 0
    private var timeStamp = ""

    private lateinit var habitViewModel: ExpenseViewModel
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        habitViewModel = ViewModelProvider(this).get(ExpenseViewModel::class.java)
    ...
    private fun addExpenseToDB() {

        //Get text from editTexts
        amount = et_expenseAmount.text.toString()
        description = et_expenseDescription.text.toString()
        ...
        if (!(amount.isEmpty() || description.isEmpty() || timeStamp.isEmpty() || drawableSelected == 0)) {
            val habit = Expense(0, amount.toInt(), expenseCategory, description, timeStamp, drawableSelected)

            //add the habit if all the fields are filled
            habitViewModel.addExpense(habit)
